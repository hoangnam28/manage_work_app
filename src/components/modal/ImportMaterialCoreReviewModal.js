...
import { importMaterialCoreData } from '../../utils/material-core-api';

const { Dragger } = Upload;
const { Title } = Typography;

const ImportMaterialCoreReviewModal = ({ open, onCancel, onSuccess, loadData }) => {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const result = await importMaterialCoreData(data);
      if (result.success) {
  ...
  return (
    <Modal
      title="Import Material Core từ Excel (Review)"
      open={open}
...
export default ImportMaterialCoreReviewModal; 